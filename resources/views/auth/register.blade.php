        ...
        <form class="mt-4" method="POST" action="{{ route('register.store') }}">
            ...
            <div class="mt-4">
                <input type="tel" 
                       ...
                       placeholder="Número telefónico @if($trialMode)({{ implode(' o ', $allowedNumbers) }})@endif" 
                       id="phone_number" 
                       name="phone_number"
                       @if($trialMode && $defaultNumber)
                           value="{{ $defaultNumber }}"
                           readonly
                       @else
                ...
                @if($trialMode)
                    <small class="text-gray-500 text-sm">
                        Modo prueba activado @if(count($allowedNumbers) > 1)(números permitidos: {{ implode(', ', $allowedNumbers) }})@endif
                    </small>
                @endif
